...
extern const uint16_t motor[23];
...
//		return motor4[(uint16_t)(fabs(pitch))];
		return motor[(uint16_t)(fabs(pitch))];
	}
	else
	{
		return 1050;
...
//		return motor2[(uint16_t)(fabs(roll))];
		return motor[(uint16_t)(fabs(roll))];
	}
	else
	{
		return 1050;
...
{
  if((Motor_Angle_X>=0)&&(Motor_Angle_X<Amplitude))
	{
		Motor_Angle_X= Motor_Angle_X +step;
	}
	else
	{
		Motor_Angle_X=Motor_Angle_X -  step;
