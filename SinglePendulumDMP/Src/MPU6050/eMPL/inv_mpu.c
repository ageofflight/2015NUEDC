...
extern sq_Queue Queue_q0;
extern sq_Queue Queue_q1;
extern sq_Queue Queue_q2;
extern sq_Queue Queue_q3;
...
//	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		/* Add user code here: Code Begine */
		
//		insert(&Queue_q0, q0);   //说明：8月3日，当摆摆动时，Pitch值输出值很小，怀疑滑动平均值滤波器将数据拉低，决定
//		insert(&Queue_q1, q1);   //先注释掉滑动平均滤波器
//		insert(&Queue_q2, q2);
//		insert(&Queue_q3, q3);
//		
//		q0=MovAve(&Queue_q0);
...
//		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
		...
		/* 根据动力摆传感器安装位置和方向，将姿态角计算改为如下 */
    *pitch = -asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3 + 0.016997;	// pitch
		*roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3 - 0.191737;	// roll
		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
